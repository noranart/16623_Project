...
- (void)initialize
{
  ...
  luaL_openlibs(L);
    ...
    NSString *documentsDirectory = [[[NSBundle mainBundle] resourcePath] stringByAppendingString:@"/Torch.framework2"];
  ...
  [self addLuaPackagePathForBundlePath:[[NSBundle mainBundle] resourcePath] subdirectory:nil];
    NSString *frameworkResourcesPath = [self bundleResourcesPathForFrameworkName:@"Torch.framework2"];
    printf("%s\n",[frameworkResourcesPath UTF8String]);

    //frameworkResourcesPath = [[NSBundle mainBundle] pathForResource:@"test" ofType:@"jpg"];
    //printf("%s\n",[frameworkResourcesPath UTF8String]);

    //NSString *frameworkResourcesPath = @"/Users/saintsol/Desktop/XOR_Example/XOR_Example/Torch.framework/Resources";
